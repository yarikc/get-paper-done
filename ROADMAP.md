...
- `#46`: compiled revision instructions; `gpd feedback-plan decide` now writes `REVISION-INSTRUCTIONS.md` after approval so revision agents apply a compact approved instruction set instead of rereading every raw concern.
- `#50`: Paper Lab A/B fixture for revision-instruction effectiveness; follow-up validation split out of #46 to prove whether compiled instructions outperform raw feedback-plan context on a public anonymized fixture.
- `#51`: research room and source inventory approval gate; RFC-016 Phase 1 backlog item for making the researched source set reviewable before outline/draft, without taking on the later global corpus.
- `#48`: guarded revision regression gate; deterministic checks now require a snapshot baseline, catch internal workflow vocabulary leakage, repeated mechanical cadence, suspicious word-count deltas without approval, missed preservation constraints, and false-positive `REVISION-CHECK.md` risk before `status`/`next` ask the user to review a regressed export as improved. Possible mechanical sentence repetition is a medium calibration warning, not a hard blocker by itself. Ratings are marked blocked when the guarded gate fails. Independent judgment-heavy scoring remains in #32.
- `#49`: regression recovery UX; now has the user-facing follow-up scope for sending a regressed paper back through recovery after #48 detects or the user reports regression.
...
43. Completed: added issue #43 Decision Sets semantic validation so malformed headings, unsupported decision tokens, missing fields, invalid user decisions, missing/unknown coverage, and overlapping concern coverage are caught before revision.
44. Completed: added issue #46 compiled revision instructions. When feedback decisions are complete, `gpd feedback-plan decide` writes `.paper/REVISION-INSTRUCTIONS.md`; `/gpd-revise` reads that compact approved instruction set first and leaves deferred/rejected/answered-no-action items out of scope.
45. Next main-line slice: finish the paused private-paper revision by approving or modifying the combined external-review feedback plan, applying accepted changes, exporting again, and using that trial to decide whether PDF/spreadsheet import handling, local HTTP provider support, or another workflow validation should follow.
