...
40. Completed: added `REVISION-CHECK.md` as the regression gate for substantive revisions after a private real-paper trial showed validator-clean edits can still degrade argument quality, evidence linkage, persona, and voice.
41. Completed: promoted paper version safety from documentation to CLI behavior after the private-paper regression. `gpd snapshot` now creates `.paper/versions/...` snapshots and `REVISION-LOG.md`; snapshots include paper artifacts, source notes, external-review captures, imported originals, and hash metadata; `gpd export` requires a current valid `REVISION-CHECK.md` before overwriting a prior export with a newer draft, then snapshots the existing `FINAL.md`; `gpd restore` restores tracked files from a snapshot after creating a safety snapshot; validation checks revision-check dimensions, score regressions, referenced version snapshots, and snapshot hashes; `gpd next` uses exported draft hashes for export freshness so touched-but-unchanged drafts do not cause false stale routing while content changes with misleading mtimes still route to export.
