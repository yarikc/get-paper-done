...
41. Completed: added issue #30 external-review provenance and the first implementation slice. `gpd review-external` now writes `.paper/EXTERNAL-REVIEW-RUN.json` and references it from `FEEDBACK-EXTERNAL.md`, recording review target, context artifacts, requested providers, current-runtime skip setting, timeout, isolated working-directory policy, safe provider command/argument shape, provider status, raw feedback paths, and the boundary that exact model/settings are provider CLI defaults or unknown unless GPD later adds provider-specific configuration. Claude paper review now defaults to `claude -p --model opus --effort high`; Gemini paper review now defaults to `gemini -p "" -m gemini-2.5-pro --output-format text --approval-mode plan --skip-trust`.
42. Next main-line slice: finish the paused private-paper revision by approving or overriding the combined external-review feedback plan, applying accepted changes, exporting again, and using that trial to decide whether PDF/spreadsheet import handling, local HTTP provider support, or another workflow validation should follow.
