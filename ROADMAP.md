...
- `#48`: guarded revision regression gate; deterministic checks now require a snapshot baseline, catch internal workflow vocabulary leakage, overloaded sentence-level repetition, repeated mechanical cadence, suspicious word-count deltas without approval, missed preservation constraints, and false-positive `REVISION-CHECK.md` risk before `status`/`next` ask the user to review a regressed export as improved. Ratings are marked blocked when the guarded gate fails. Independent judgment-heavy scoring remains in #32.
