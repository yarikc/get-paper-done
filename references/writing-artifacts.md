...
When users review an exported paper, `.paper/exports/FINAL.md` is the reading copy and `.paper/DRAFT.md` remains the editable source of truth. Inline comments on `FINAL.md` or a companion feedback file should be captured with `gpd feedback`, approved through `/gpd-feedback`, applied to `DRAFT.md` by `/gpd-revise`, and regenerated by `/gpd-export`.

`/gpd-feedback` is the normal human approval loop for `FEEDBACK-PLAN.md`. It should show one concern at a time, present one selection list with `approve`, `modify`, `defer`, and `reject`, and record `User Decision` plus `User Constraint`. If the user selects `modify`, ask one follow-up for the constraint or instruction. It does not revise the draft.
