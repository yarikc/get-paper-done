...
External review workflows must store reviewer-specific captures under `.paper/feedback-external/`, write the active combined review to `.paper/FEEDBACK-EXTERNAL.md`, and write a concern-first approval queue to `.paper/FEEDBACK-PLAN.md`. They must not change `.paper/DRAFT.md`.

When users review an exported paper, `.paper/exports/FINAL.md` is the reading copy and `.paper/DRAFT.md` remains the editable source of truth. Inline comments on `FINAL.md` or a companion feedback file should be captured with `gpd feedback collect`, approved through `/gpd-feedback`, applied to `DRAFT.md` by `/gpd-revise`, and regenerated by `/gpd-export`.

`/gpd-feedback` is the normal human approval loop for `FEEDBACK-PLAN.md`. It should show one concern at a time, present one selection list with `approve`, `modify`, `defer`, `reject`, and `answered_no_action`, and record `User Decision` plus `User Constraint`. If the user selects `modify`, ask one follow-up for the constraint or instruction. It does not revise the draft.
...
`/gpd-review --external` can request feedback from installed external AI CLIs or collected review files. Use `/gpd-review --external --models claude,gemini,codex` to limit reviewers. Opencode is intentionally unsupported for paper review until a future calibration proves it reliable for this workflow.
