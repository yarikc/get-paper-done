...
Get Paper Done is a strong writing workflow framework, but the latest private-paper trial exposed a severe safety gap: a strong reviewed paper version could be overwritten during revision/export because the workflow had a regression-check artifact but no enforced recoverable baseline. That failure is now addressed in the CLI: `gpd snapshot` creates paper-local `.paper/versions/...` snapshots and `REVISION-LOG.md`, snapshots include source notes, external-review captures, imported originals, and hash metadata, `gpd restore` restores tracked files after creating a safety snapshot, `gpd export` requires a current valid `REVISION-CHECK.md` before overwriting a prior export with a newer draft, `gpd next` uses exported draft hashes for export freshness, and `REVISION-CHECK.md` validation now checks required dimensions, score-regression consistency, referenced snapshot existence, and snapshot hashes. The project still has a simpler product-story README, a concise newcomer `START-HERE` guide, CLI `gpd next` guidance, the exported-paper comment loop, artifact contracts, schema validation, workflow consistency tests, scenario routing tests, semantic gates, realistic examples, import hardening, mandatory `/gpd-grill`, reusable context packs, release/update guidance, and package-boundary hygiene tests.
...
| Installable tool maturity | 9.1/10 | CLI covers install/update/doctor/init/import/export/next/status/validate plus `validate-artifact`, `review-external`, `snapshot`, and `restore`; export protects existing `FINAL.md` with a revision-check guard and overwrite snapshot; revision-check validation is stronger. Missing local HTTP server support, PDF/spreadsheet extraction, local project install mode, public/team distribution policy, and broader version-safety UX keep it below a complete product. |
