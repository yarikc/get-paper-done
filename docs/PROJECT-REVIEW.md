...
| Installable tool maturity | 9.15/10 | CLI covers install/update/doctor/init/import/export/next/status/validate plus `validate-artifact` and `review-external`; `STATE.json.grill` now prevents skipping pre-brief ambiguity removal; import now has dry-run inventory, classification counts, warning output, skip-threshold control, deterministic draft-candidate ranking, `.docx` canonical-draft text extraction, unverified source-reference triage, and version/source indexing; export has a regression test for pre-body draft sections; external review can be collected from files/stdin or invoked through selected installed provider CLIs, with Claude, Codex, and opencode calibrated on synthetic public papers. Missing Gemini real-capture authentication, local HTTP server support, PDF/spreadsheet extraction, local project install mode, and public/team distribution policy keep it below a complete product. |
| Documentation | 9.5/10 | README is now a shorter product front door instead of a reference manual, and `docs/START-HERE.md` gives a first-paper path centered on examples, install, `gpd next`, and `/gpd-progress`. Dense mechanics remain in reference docs. Still needs real newcomer feedback before more onboarding changes. |
...
- Claude, Codex, and opencode provider paths are calibrated on synthetic public papers. Gemini argument shape is corrected, but real capture is blocked until local Gemini authentication is completed.
...
- The README now acts as a concise product front door, while `docs/START-HERE.md` gives a first-paper path through examples, install, setup, `gpd next`, `/gpd-progress`, profile, audience, workflow commands, feedback, and import.
...
5. External review is now tool-wrapped for collection and first-pass installed-provider invocation. Claude, Codex, and opencode are calibrated; Gemini still needs authentication-backed real capture, and local HTTP providers such as Ollama, LM Studio, and llama.cpp remain workflow/manual.
...
1. Decide whether to complete Gemini authentication-backed calibration, continue import hardening with PDF/spreadsheet handling, or pause broad hardening for another real-paper workflow validation.
2. Get real newcomer feedback on the slimmer README, `START-HERE`, `gpd next`, and `/gpd-progress` before adding more onboarding surface.
...
7. Calibrate `gpd review-external --models gemini` after local authentication before broadening to local HTTP providers.
...
Both passed after the mandatory/re-enterable grill-state gate, reusable context-pack install fix, and documentation updates.
