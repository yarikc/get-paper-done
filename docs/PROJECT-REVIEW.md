...
- Claude and Codex provider paths are calibrated on synthetic public papers. Gemini argument shape is corrected, but real capture is blocked until local Gemini authentication is completed. Opencode is intentionally unsupported for paper review.
...
5. External review is now tool-wrapped for collection and first-pass installed-provider invocation with visible provider progress. Claude and Codex are calibrated; Gemini still needs authentication-backed real capture; Opencode is intentionally unsupported for papers; local HTTP providers such as Ollama, LM Studio, and llama.cpp remain workflow/manual.
