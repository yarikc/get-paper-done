...
| Installable tool maturity | 9.15/10 | CLI covers install/update/doctor/init/import/export/next/status/validate plus `validate-artifact`, `review-external`, `revise`, `snapshot`, and `restore`; controlled revision now has a snapshot preflight and status/next restore visibility; generated artifact writes use temp-file-plus-rename updates; export protects existing `FINAL.md` with a revision-check guard and overwrite snapshot; revision-check validation is stronger. Missing local HTTP server support, PDF/spreadsheet extraction, local project install mode, public/team distribution policy, and broader version-safety UX keep it below a complete product. |
