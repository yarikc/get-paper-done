...
**Historical, pre-RFC-017 snapshot: 9.25/10 as a writing framework; 9.1/10 as an installable private-repo tool**

These numeric ratings are retained as dated calibration notes. They are not workflow authority and should not be used to decide whether a paper improved; RFC-017 replaces rating-driven routing with accepted baselines, candidate comparison, and human approval gates.

Get Paper Done is a strong writing workflow framework, but the latest private-paper trial exposed two severe safety gaps. The first was version loss: a strong reviewed paper version could be overwritten during revision/export because the workflow had a regression-check artifact but no enforced recoverable baseline. The GPD-controlled path now addresses that failure: `gpd revise` snapshots the current paper before controlled revision and prints the restore command, `gpd snapshot` creates paper-local `.paper/versions/...` snapshots and `REVISION-LOG.md`, snapshots include source notes, external-review captures, imported originals, and hash metadata, `gpd status`/`gpd next` surface the latest restore command, `gpd restore` restores tracked files after creating a safety snapshot, `gpd export` requires a current valid `REVISION-CHECK.md` before overwriting a prior export with a newer draft, `gpd next` uses exported draft hashes for export freshness, and `REVISION-CHECK.md` validation now checks required dimensions, score-regression consistency, referenced snapshot existence, and snapshot hashes. The second was quality false confidence: an externally influenced revision could make the paper worse while a self-graded `REVISION-CHECK.md` reported every dimension improved. The guarded revision gate now catches deterministic false-positive signals, including missing snapshot baselines, internal workflow vocabulary leaking into reader-facing text, suspicious word-count deltas, missed preservation constraints, and `REVISION-CHECK.md` claiming no regression while guarded checks fail; it also surfaces possible mechanical sentence repetition as a medium calibration warning rather than a hard blocker. `gpd status` and `gpd next` mark ratings blocked and route away from user review when blocking guarded checks fail. Independent judgment-heavy revision scoring and full recovery UX remain open in #32 and #49. Direct filesystem edits outside the GPD revise preflight can still bypass CLI enforcement, so the editor agent and `/gpd-revise` command now treat edit-without-snapshot as a workflow violation. The project still has a simpler product-story README, a concise newcomer `START-HERE` guide, CLI `gpd next` guidance, the exported-paper comment loop, artifact contracts, schema validation, workflow consistency tests, scenario routing tests, semantic gates, realistic examples, import hardening, mandatory `/gpd-grill`, reusable context packs, release/update guidance, and package-boundary hygiene tests.

The installable tool is now at the 9/10 private-repo threshold, but still not finished as a distributable product. Import now has better dry-run inventory, skip-threshold control, deterministic draft-candidate ranking, first-pass `.docx` canonical-draft text extraction, unverified source-reference triage, and version/source indexing, but it is still preservation-first rather than extraction-rich for PDFs, spreadsheets, comments, tracked changes, and deep version history. External review now has a safe CLI collector plus a first `--models` provider-invocation path for installed CLIs; it shows provider-level progress while reviewers run, stores reviewer-specific captures, deduplicates combined feedback, writes provenance, and converts feedback into a concern-first decision queue. Claude, Codex, and Gemini are calibrated for paper review when their local CLIs are authenticated, and Opencode is intentionally unsupported for papers after local calibration proved unreliable. Local HTTP servers remain workflow/manual. The private-repo release/update path is documented and test-backed, while public or team distribution remains a later decision.

## Historical Ratings

| Area | Rating | Rationale |
|------|--------|-----------|
| Framework design | 9.25/10 | Strong staged model, explicit paper memory, normalized classification, mandatory grill before brief, research compression, strategy gate, audience system, reader feedback capture, feedback approval, governance/control-paper guidance, and multiple examples. Rating is reduced because a real paper trial exposed that revision quality controls were not enough without enforced version safety. Snapshot/export protection is now implemented, but needs more real-paper calibration. |
| Installable tool maturity | 9.1/10 | CLI covers install/update/doctor/init/import/export/next/status/validate plus `validate-artifact`, `review-external`, `revise`, `snapshot`, and `restore`; controlled revision now has a snapshot preflight and status/next restore visibility; generated artifact writes use temp-file-plus-rename updates; export protects existing `FINAL.md` with a revision-check guard and overwrite snapshot; revision-check validation is stronger. Missing local HTTP server support, PDF/spreadsheet extraction, local project install mode, public/team distribution policy, and broader version-safety UX keep it below a complete product. |
...
- Generated paper artifacts now write through a temporary file and rename path, reducing the risk of partially written state or metadata files after interrupted writes.
- `gpd snapshot`, `.paper/versions/`, and `REVISION-LOG.md` now preserve recoverable baselines before risky paper changes, including paper-local source notes, external-review captures, imported originals, and hash metadata.
- `gpd status` and `gpd next` now surface the latest restore command when a snapshot exists, reducing reliance on memory during recovery.
- `gpd restore` now restores tracked files from a snapshot after creating a safety snapshot of the current state.
- `gpd export` now requires a current valid `REVISION-CHECK.md` before overwriting an existing `FINAL.md` with a newer draft, then snapshots the prior export.
- `REVISION-CHECK.md` now protects substantive revision quality by comparing before/after thesis clarity, argument flow, evidence support, audience fit, persona and voice, ask clarity, and substance preservation before export; validation now checks required dimensions, snapshot references, and snapshot hashes.
- Guarded revision semantic checks now protect against the demonstrated false-positive pattern where `REVISION-CHECK.md` says a revision improved while deterministic signals show regression. Status/next no longer route to user review when these checks fail.
- `examples/software-supply-chain-evidence-pack` now demonstrates `FEEDBACK-READER.md` and `FEEDBACK-PLAN.md` end to end: low evidence/ask-clarity scores triggered a plan, approval gate, backward routing to research, and a stronger final memo.
...
- `gpd next` and `gpd status` now compare the current draft hash to the last exported draft hash for export freshness, avoiding false stale routing from touched-but-unchanged drafts while still catching content changes with misleading mtimes.
...
- Approved feedback now compiles into `REVISION-INSTRUCTIONS.md`, so revision agents get one compact, approved instruction set and deferred/rejected items stay explicit out of scope.
...
- `gpd review-external` preserves any prior feedback plan under `Prior Feedback Plan`, so rerunning external review does not silently erase user decisions.
- The drafting agent now has the same revision-safety discipline as the editor for redrafting existing content: replacing or materially editing an existing `DRAFT.md` requires a snapshot preflight.
...
- `examples/software-supply-chain-evidence-pack` is the current showcase example: a pre-registered real-public-source workspace with README, EXPECTED-FINDINGS, PAPER-CONTEXT, DECISIONS, FEEDBACK-READER, FEEDBACK-PLAN, CISA, NIST, SLSA, OpenSSF, OWASP, and NCSC source URLs, claim-support metadata, control-process revision, high-risk scope definition, AI runtime evidence, decision rules, validation model, process-burden handling, fact-check, review, and exported citations.
...
6. External review is now tool-wrapped for collection and first-pass installed-provider invocation with visible provider progress. Claude, Codex, and Gemini are calibrated for paper review when their local CLIs are authenticated; Opencode is intentionally unsupported for papers; local HTTP providers such as Ollama, LM Studio, and llama.cpp remain workflow/manual.
7. Release readiness remains private-repo oriented. The local/private release path is now documented and test-backed, but a public or team-facing release still needs a distribution decision.
8. Classification is not yet used by the CLI routing engine. It is enforced in schema, templates, workflows, docs, and examples, but `gpd status` still routes mostly from artifact state, strategy state, mtimes, exported draft hashes, review/fact-check state, and export state.
9. Feedback files intentionally remain ignored rather than archived in tracked docs. Useful feedback should be converted into anonymized issues, examples, fixtures, or documentation updates; raw feedback should not become package material.
10. Semantic validation does not yet drive routing. `gpd validate --semantic` can detect paper-quality failures, but `gpd next` still routes from artifact presence, strategy state, mtimes, exported draft hashes, fact-check/review verdicts, feedback-plan approval, and export state. Decide whether semantic failures should become routing blockers after the next real-paper trial.
11. Context packs are installed and documented, but there is no CLI discovery command such as `gpd list-contexts`. For now, users inspect `contexts/` directly and `/gpd-grill` treats reusable context as optional.

## Recommended Next Work

1. Validate snapshot/export/restore protection on the next real paper before doing another broad revision pass.
2. Run the next real-paper trial, then decide whether semantic validation and classification should affect `gpd next` routing.
3. Get real newcomer feedback on the slimmer README, `START-HERE`, `gpd next`, and `/gpd-status` before adding more onboarding surface.
4. Harden `gpd import` around non-DOCX source conversion only when real use exposes the need.
5. Add richer fixture workspaces only when they represent a new failure mode, not just more routing signals.
6. Expand semantic lint-style checks or classification-driven validators only where they are concrete enough to be useful; defer noisy heuristics until examples exist.
7. Decide whether public/team distribution is needed, and if so define publishing, tagging, and support policy.
8. Broaden external-review provider support only after another real paper trial proves a concrete need; local HTTP providers remain manual for now.
9. Decide whether `contexts/` needs `gpd list-contexts` after more than one reusable context pack exists.
...
Both passed after the snapshot, restore, export-guard, hash-validation, and documentation updates.
