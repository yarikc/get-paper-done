...
Get Paper Done is a coherent, useful writing workflow framework with a practical CLI wrapper and a stronger documentation surface than the initial release. The project now has a clearer README, artifact contracts, JSON schema validation, workflow consistency tests, scenario routing tests, and CLI support for validating individual artifacts.
...
| Test coverage | 8.6/10 | Tests now cover core CLI behavior, artifact contracts, malformed JSON, enum drift in state/research, exact audience scorecard dimensions, malformed headings, workflow reference consistency, and scenario routing for backward/incremental refresh. Still missing real paper fixtures and end-to-end paper runs. |
...
3. Add richer fixture workspaces that exercise review, fact-check, feedback-plan, and export-ready states.
