...
If thesis, audience, classification, reader promise, paper job, scope, desired outcome, narrative spine, or key terms are unclear, recommend `/gpd-grill` before `/gpd-brief` and before showing the three post-import choices. The import stage should not reduce an imported paper into a brief until ambiguity has been challenged with the user.

If `STRATEGY.md` status is `Revise Before Drafting` or `No-Go`, show the block reason and primary blocker. Set suggested next command to `/gpd-grill` when the blocker is unresolved thesis, audience, reader promise, narrative spine, or missing outcome; otherwise set suggested next command to `/gpd-brief`. Do not recommend research, outline, review, fact-check, or export unless the user explicitly overrides the strategy block.
...
- Ambiguous imported author intent routes to `/gpd-grill` before `/gpd-brief`, research, outline, or review
