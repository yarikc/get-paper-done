...
- If they add comments directly to `FINAL.md` or a `.feedback` file, run `gpd feedback` from the paper directory, then `/gpd-feedback`.
...
- Recommend the next review path explicitly. After substantive revision/export,
  default to user review before external review because the user must confirm
  intent, voice, posture, and calibration before an external reviewer amplifies
  or redirects the paper. If the user accepts the export, external review becomes
