...
- External-review feedback plans now decompose captured HIGH/MEDIUM/LOW concerns and suggested changes into separate numbered approval items instead of collapsing a whole review into one generic item, with a decision view, proposed fix, guardrail, and `User Override` field for author changes before revision.
- External-review runs now store each reviewer capture under `.paper/feedback-external/`, keep `FEEDBACK-EXTERNAL.md` and `FEEDBACK-PLAN.md` as the active combined view, deduplicate overlapping reviewer concerns, and print the combined decisions at command completion.
- External-review runs now write `.paper/EXTERNAL-REVIEW-RUN.json` so provider/file review provenance records the review target, context artifacts, requested providers, current-runtime skip setting, timeout, isolated working-directory policy, safe provider command/argument shape, provider status, raw feedback paths, and exact-model policy.
- Claude external-review provider default changed from `claude -p` to `claude -p --model opus --effort high` for final paper-review quality.
- Gemini external-review provider default changed from `gemini -p ""` to `gemini -p "" -m gemini-2.5-pro --output-format text --approval-mode plan --skip-trust` for stable Pro read-only paper review in isolated provider workspaces.
- Reader-feedback plans generated by `gpd feedback` now include concise decision, proposed fix, guardrail, and the same `User Override` field.
