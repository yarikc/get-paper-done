...
- `gpd feedback-plan decide` now compiles `.paper/REVISION-INSTRUCTIONS.md` after the final required decision, giving `/gpd-revise` a compact approved instruction set with deferred/rejected items explicitly out of scope.
- Guarded revision semantic checks now catch deterministic regression signals after substantive revisions, including missing snapshot baselines, internal workflow vocabulary leaking into reader-facing text, suspicious word-count deltas, missed preservation constraints, and `REVISION-CHECK.md` false-positive risk when the check claims no regression while guarded checks fail. Possible mechanical sentence repetition is surfaced as a medium calibration warning rather than a hard blocker, because repetition can be intentional rhythm, definition, or necessary scope language.
