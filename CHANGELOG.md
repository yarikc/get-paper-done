...
- `gpd feedback-plan list|review|decide` lets users review the feedback queue inside the CLI/TUI, inspect one concern at a time, and record `approve`, `modify`, `defer`, or `reject` decisions before revision.
...
- Reader-feedback plans generated by `gpd feedback` now use the same concern-first approval queue as external-review feedback plans.
...
- `FEEDBACK-PLAN.md` status is now validated against the same approval vocabulary, and exported-paper comment guidance now routes users through `gpd feedback` followed by `/gpd-feedback`.
